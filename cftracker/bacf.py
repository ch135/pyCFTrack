...
class BACF(BaseCF):
    def __init__(self, config=BACFConfig()):
        super(BACF).__init__()
        self.cell_size=config.cell_size                     # 4
        self.cell_selection_thresh=config.cell_selection_thresh # 0.75**2
        self.search_area_shape = config.search_area_shape
        self.search_area_scale=config.search_area_scale     # 5
        self.filter_max_area = config.filter_max_area
        self.interp_factor=config.interp_factor     # 0.015??
        self.output_sigma_factor = config.output_sigma_factor       # 1/16 ??
        self.interpolate_response =config.interpolate_response      # 4
        self.newton_iterations =config.newton_iterations            # 5
        self.number_of_scales =config.number_of_scales              # 1
        self.scale_step = config.scale_step                         # 1.01
        self.admm_iterations = config.admm_iterations               # 2
        self.admm_lambda = config.admm_lambda                       # 0.01
    ...
    def init(self,first_frame,bbox):
        ...
        if self.search_area < self.cell_selection_thresh*self.filter_max_area:    # 搜索区域< 0.75*50的正方形
            ...
            self.cell_size = int(min(self.feature_ratio,max(1,int(np.ceil(np.sqrt(self.w*self.search_area_scale/(self.cell_selection_thresh*self.filter_max_area)*self.h*self.search_area_scale/(self.cell_selection_thresh*self.filter_max_area)))))))
            self.feature_ratio=self.cell_size
            self.search_area = (self.w / self.feature_ratio * self.search_area_scale) * \
                               (self.h / self.feature_ratio * self.search_area_scale)
        ...
        if self.search_area_shape=='proportional':
            self.crop_size=(int(self.base_target_sz[0]*self.search_area_scale),int(self.base_target_sz[1]*self.search_area_scale))          # crop_size:

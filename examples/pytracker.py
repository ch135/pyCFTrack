...
class PyTracker:
    ...
    def tracking(self,verbose=True,video_path=None):
        ...
        for idx in range(len(self.frame_list)):
            if idx != 0:
                ...
                if verbose is True:
                    ...
                    all_FPS.append(fps)
                    all_APCE.append(apce)
                    all_PSR.append(psr)
                    all_Fmax.append(F_max)

                    # 照片/添加的文字/左上角坐标/字体/字体大小/颜色/字体粗细
                    cv2.putText(show_frame, 'FPS:' + str(np.mean(all_FPS))[:5], (375, 25), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (255, 0, 0), 2)
                    cv2.putText(show_frame, 'APCE:' + str(np.mean(all_APCE))[:5], (375, 50), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (0, 0, 255), 2)
                    cv2.putText(show_frame, 'PSR:' + str(np.mean(all_PSR))[:5], (375, 75), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (255, 0, 0), 2)
                    cv2.putText(show_frame, 'Fmax:' + str(np.mean(all_Fmax))[:5], (375, 100), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (255, 0, 0), 2)
        ...
        return np.array(poses), str(np.mean(all_FPS))[:5], str(np.mean(all_APCE))[:5], str(np.mean(all_PSR))[:5], str(np.mean(all_Fmax))[:5]
